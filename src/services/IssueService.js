...
// ──────────────────────────────────────────────
// We do NOT use statusName from applicableIssue anymore
// Only applicantFormId presence indicates already applied
// ──────────────────────────────────────────────
function alreadyApplied(issue) {
  return Boolean(issue.applicantFormId);
}
...
function classify(issue) {
  ...
  if (isDebenture(issue)) {
    return {
      ...
      kind: 'UNKNOWN',
      eligible: false,
      skipReason: 'Skipped: Debenture',
      alreadyApplied: alreadyApplied(issue)
    };
  }
  ...
  if (!isOrdinaryShares(issue)) {
    return {
      ...
      kind: 'UNKNOWN',
      eligible: false,
      skipReason: 'Skipped: Not Ordinary Shares',
      alreadyApplied: alreadyApplied(issue)
    };
  }
  ...
  if (!isGeneralPublic(issue)) {
    return {
      ...
      kind: 'UNKNOWN',
      eligible: false,
      skipReason: 'Skipped: Not General Public',
      alreadyApplied: alreadyApplied(issue)
    };
  }
  ...
  return {
    ...
    kind: 'UNKNOWN',
    eligible: false,
    skipReason: 'Skipped: Not IPO/FPO or Right Share',
    alreadyApplied: alreadyApplied(issue)
  };
}
